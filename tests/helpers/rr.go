...
const (
	...
	// and the emulator adds its own pull latency, so a record can lag the rpc
	// call that caused it by a wide margin.
	...
	// appear. Delivery from the emulator normally lands well inside a second.
...
// subscription exists and the pipeline is ready to take calls.
...
// Start wipes the emulator, registers the plugins, boots the container and
// waits for the probe, if any, to answer. Errors arriving on the container
// channel are reported through t.Errorf and stop the container, but they do not
// abort the test.
//
...
func Start(t *testing.T, cfgPath string, plugins []any, opts ...Option) (*RR, func()) {
	...
	require.NoError(t, CleanEmulator(), "wiping the emulator")
